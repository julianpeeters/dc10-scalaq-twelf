...
given `1.7.1+`: Renderer["Twelf 1.7.1+", Error, List[Statement]] =
  new Renderer["Twelf 1.7.1+", Error, List[Statement]]:
    ...
    override def version: "Twelf 1.7.1+" =
      "Twelf 1.7.1+"
    ...
    private def renderType[Z, T, Y](tpe: Term.TypeLevel[T, Z]): String =
      tpe match
        // application
        ...
        case Term.TypeLevel.App.InfixPi(qnt, tfun, a, tb, z) => s"{${renderValue(a).toUpperCase}:${renderType(a.tpe)}} ${renderType(tfun)} ${renderType(tb)}"
        // primitive
        ...
        case Term.TypeLevel.Var.UserDefinedType(q, s, i, z) =>
          if z.tpe == Term.TypeLevel.Var.UnitType(None)
          then s.toLowerCase()
          else s.toLowerCase() + " " + renderValue(z).toUpperCase()
    ...
    private def renderTypeDef(typeDef: Statement.TypeDef): String =
      typeDef match
        case d@Alias(i, s) => d.tpe.impl.fold(
          ...
          s"${d.tpe.nme.foldLeft("")((acc, c) => if c.isUpper then acc :+ '-' :+ (c.toLower) else acc :+ c)} : ${renderType(i)}"
        )
        case d@Match(i, s) =>
    ...
    private def renderValue[Z, T, X](value: Term.ValueLevel[T, Z]): String =
      value match 
        ...
        case Term.ValueLevel.Var.UserDefinedValue(q, s, t, i) => t match
          case Term.TypeLevel.App.App1(qnt, tfun, targ, dep) => ???
          case Term.TypeLevel.App.App2(qnt, tfun, ta, tb, dep) => ???
          case Term.TypeLevel.App.App2T(qnt, tfun, ta1, ta2, tb, dep) => ???
          case Term.TypeLevel.App.App3(qnt, tfun, ta1, ta2, tb, dep) => ???
          case Term.TypeLevel.App.Infix(qnt, tfun, ta, tb, dep) => ???
          case Term.TypeLevel.App.InfixPi(qnt, tfun, a, tb, dep) => ???
          case Term.TypeLevel.Lam.Function1Type(qnt, dep) => ???
          case Term.TypeLevel.Lam.Function2Type(qnt, dep) => ???
          case Term.TypeLevel.Var.BooleanType(qnt, dep) => ???
          case Term.TypeLevel.Var.IntType(qnt, dep) => ???
          case Term.TypeLevel.Var.StringType(qnt, dep) => ???
          case Term.TypeLevel.Var.UnitType(qnt) => ???
          case Term.TypeLevel.Var.ListType(qnt, dep) => ???
          case Term.TypeLevel.Var.OptionType(qnt, dep) => ???
          case Term.TypeLevel.Var.SomeType(qnt, dep) => ???
          case Term.TypeLevel.Var.TupleType(qnt, dep) => ???
          case Term.TypeLevel.Var.UserDefinedType(qnt, nme, impl, dep) => s"$s"
    ...
    private def renderValueDef(valueDef: Statement.ValueDef): String =
      valueDef match
        ...
        case d@Statement.ValueDef.Val(_, _)  =>
          d.value.impl.fold(
            s"${d.value.nme.foldLeft("")((acc, c) => if c.isUpper then acc :+ '-' :+ (c.toLower) else acc :+ c)} : ${renderType(d.value.tpe)}"
          )(
            i =>
